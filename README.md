...
differing only in one deleted observation — we find that a downstream event's cached rows can answer
from the deleted value even though the served text never states it (99:0 on Qwen3-8B). We
...
Set `SPRAG_MODEL_PATH` to a local HuggingFace directory. The paper's recent models are **Qwen3-8B**
(primary), **Ministral-3-8B**, **Gemma-4-12B**, and **Llama-3.1-8B**. 2026 / long-context
checkpoints may need chunked prefill (`export SPRAG_ENCODE_CHUNK=4096`) and a recent `transformers`.
